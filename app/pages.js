...
export default function (node_Id) {
  $.getJSON('/api/pages',json => {
    ...
    for (let i = 0; i < pageJSON.length; i++) {
      ...
      page = '<div class="pageDiv col-md-2"><div class="pageTitle">' + pageJSON[i].name + '</div><div class="pageScreenshot"><img style="width:' + ((pageJSON[i].hits/maxCount) * 100) + '%;" src="' + screenshotUrl + '"><div class="pageCount">ViewCount: ' + pageJSON[i].hits + '</div></div></>';
      $pages.append(page);
    }
